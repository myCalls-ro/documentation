...
{/* - If you opt for the auth route, you need to specify the username and password in the next step when importing the number to myCalls.
   <Frame>
     <img
...
- Currently myCalls SIP server does not have a static IP, so if you opt for the IP route, you need to whitelist all the IP addresses in the range like following:
...
- Here you will specify myCalls SIP server address as the origination SIP URI: ```sip:5wed79aeavr.sip.livekit.cloud ``` 
...
## Step 3: Import numbers to myCalls

Now the number is set up with your elastic SIP trunking, you need to import the number to myCalls so that we will know how to route the call.
...
3. Click “Integrate SIP trunk”
   ...
   <Frame>
     <img
       src="/resources/twillio-import-mycalls-sip1.png"
       alt="Import numbers to myCalls"
     />
   </Frame>
...
```
   <Frame>
     <img
       src="/resources/twillio-mycalls-auth-integration.png"
...
{/* - If you opted for auth route select Username and password.
   ...
   <Frame>
     <img
       src="/resources/twillio-mycalls-credentials-auth-type.png"
       alt="Twillio myCalls Credentials Auth Type"
     />
   </Frame> */}
...
```
   <Frame>
     <img
       src="/resources/twillio-mycalls-credentials-ip-type1.png"
       alt="Twillio myCalls Credentials IP Type"
     />
   </Frame> 
...
7. Select the country of where your SIP trunk is located. 
    ...
    <Frame>
      <img
        src="/resources/twillio-mycalls-sip-country.png"
        alt="Twillio myCalls SIP Country"
      />
    </Frame>
